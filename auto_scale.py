...
file_path = '/home/kimpass189/code/docker/data/cpu.csv'
...
def scale():
    ...
    try:
        while True:
            ...
            scale_time.append(now.strftime('%Y-%m-%d %H:%M:%S'))
            scale_done.append("No")
        ...
        if os.path.exists(file_path): # 파일이 이미 있다면?
            ...
            new_data = pd.DataFrame({'time' : scale_time,'cpu' : cpu_list, 'scale' : scale_done})
            con_data = pd.concat([data, new_data])
            con_data.to_csv(file_path, index = False)
        else: # 파일을 처음 만드는 경우
            data = pd.DataFrame({'time' : scale_time, 'cpu' : cpu_list, 'scale' : scale_done})
